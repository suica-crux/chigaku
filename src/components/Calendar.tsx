  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push(
      <div key={d} className="p-1">
        <div className={`w-full h-full flex flex-col items-center justify-center ${dayClass} rounded-sm p-1`}>
          <span className="font-medium">{d}</span>
          {info?.time && (
            <span className="text-xs mt-1 text-center">{startTime}<br />{endTime}</span>
          )}
        </div>
      </div>
    );
